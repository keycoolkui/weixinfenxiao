		...
		<table class="table table-hover">
			...
			<?php  if(is_array($permission)) { foreach($permission as $row) { ?>
				<tr <?php  if(!empty($_GPC['fromuid']) && $_GPC['fromuid']== $row['uid']) { ?>style="background:#dddddd;"<?php  } ?>>
					...
					<td>
						<label for="radio_<?php  echo $row['uid'];?>_1" class="radio-inline" style="padding-top:0"><input type="radio" name="role[<?php  echo $row['uid'];?>]" targetid="<?php  echo $row['uid'];?>" id="radio_<?php  echo $row['uid'];?>_1" value="operator" <?php  if(empty($row['role']) || $row['role'] == 'operator') { ?> checked<?php  } ?> /> 操作员</label>
						<label for="radio_<?php  echo $row['uid'];?>_2" class="radio-inline" style="padding-top:0"><input type="radio" name="role[<?php  echo $row['uid'];?>]" targetid="<?php  echo $row['uid'];?>" id="radio_<?php  echo $row['uid'];?>_2" value="manager" <?php  if($row['role'] == 'manager') { ?> checked<?php  } ?> /> 管理员</label>
					</td>
