...
<!-- saved from url=(0065)http://www.shshiw.com/weixin/?from=singlemessage&isappinstalled=0 -->
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<meta charset="utf-8">
	<title><?php  echo $gz['title'];?></title>
	...
	<style>
		...
		*:first-child+html .clearfix{zoom:1;}
		...
		.header{height:120px;color:#FFF;position:relative;overflow:hidden;background-repeat:no-repeat;background-position:right top;background-size:auto 54px;-webkit-box-shadow:0 2px 10px 2px #d3d3d3;box-shadow:0 2px 10px 2px #d3d3d3;margin-bottom:10px;}
		.header .left{float:left;height:100%;background-repeat:no-repeat;background-position:left 0;background-size:246px 177px;}
		...
		.header .left h1{padding:10px 0 2px 15px;font-weight:normal;font-size:20px;line-height:23px;}
		.header .left span{font-size:13px;line-height:18px;padding:0 0 0 15px;}
		.header{background:url('<?php  echo $_W['attachurl'];?><?php  echo $gz['banner_pic'];?>') repeat-y;}
...
<body>
 	<div class="wrapper" id="wrapper">
      <ul class="header">
          <?php  echo $gz['banner_desc'];?>
      </ul>
      <div class="tj_box">
          <?php  if(is_array($list)) { foreach($list as $item) { ?>
              <a style="text-decoration:none;" href="<?php  echo $item['i_url'];?>">
                <dl class="clearfix">
                  <dd><div style="position: relative;padding-right: 110px;"><h3 class="font14"><?php  echo $item['title'];?></h3><p><?php  echo $item['i_desc'];?></p><span style="display: inline-block;width: 60px; height: 33px;line-height: 33px;background-color: #44b549;border-radius: 5px;text-align: center;color:#fff;position: absolute;top: 50%;right:55px;margin-top: -17px;">+关注</span></div></dd>
                  <dt class="tj_base"><img src="<?php  echo $_W['attachurl'];?><?php  echo $item['icon'];?>"></dt>
                </dl>
               </a>
          <?php  } } ?>
