...
//-----------------------------------web端
	...
	public function doWebfansmanager(){
		...
		if($op=='display'){
			$pindex = max(1, intval($_GPC['page']));
			$psize = 20;
			$list = pdo_fetchall("select sale.*,mc_members.credit1,mc_members.avatar from ".tablename('eso_sale_member'). " sale left join ".tablename('mc_members'). " mc_members on sale.uid=mc_members.uid where sale.uniacid = ".$_W['uniacid']." ORDER BY sale.id DESC limit ".($pindex - 1) * $psize . ',' . $psize);
			$total = pdo_fetchcolumn("select count(id) from". tablename('eso_sale_member'). "where uniacid =".$_W['uniacid']);;
			$pager = pagination1($total, $pindex, $psize);
        ...
        if($op == 'addbusiness') {
            include "mobile/phpqrcode.php";//引入PHP QR库文件
            $value = $_W['siteroot']."app/".$this->mturl('register',array('agent'=>0));
            $errorCorrectionLevel = "L";
            $matrixPointSize = "4";
            $imgname = "business". $_W['uniacid'] .".png";
            $imgurl = "../addons/eso_sale/style/images/share/$imgname";
            QRcode::png($value, $imgurl, $errorCorrectionLevel, $matrixPointSize);

            include $this->template('fansmanager_addbusiness');
            exit;
        }
...
//			$count3=0;
//		}
        $tablename = $profile['agent'] == 0 ? tablename('eso_sale_member') : tablename('eso_sale_order');
        $list = pdo_fetchall('select mem.id, mem.realname, (select count(id) from '. $tablename .' where shareid=mem.id) count from ' . tablename('eso_sale_member') . ' mem where mem.shareid = ' . $profile['id']);
