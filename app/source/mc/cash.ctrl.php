...
if(!empty($type)) {
	...
	if($type == 'wechat') {
		...
		if(!empty($plid)) {
			...
			$tag['acid'] = $_W['acid'];
			$eso_tag = '微信支付';
			pdo_update('core_paylog', array('openid' => $_W['openid'], 'tag' => iserializer($tag), 'eso_tag'=>$eso_tag, 'createtime'=>date('Y-m-d H:i:s')), array('plid' => $plid));
		}
		load()->model('payment');
		load()->func('communication');
		...
		header("location: ../payment/wechat/pay.php?i={$_W['uniacid']}&auth={$auth}&ps={$sl}&openid={$_W['openid']}");
		exit();
	}
	if($type == 'credit') {
